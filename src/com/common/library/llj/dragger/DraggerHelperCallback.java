...
public class DraggerHelperCallback extends ViewDragHelper.Callback {
     ...
     */
    public DraggerHelperCallback(DraggerView draggerView, DraggerHelperListener draggerListener) {
    ...
    public boolean tryCaptureView(View child, int pointerId) {
        if (child.equals(draggerView.getDragView())) {
            switch (draggerView.getDragPosition()) {
                case LEFT:
                    return draggerView.getDragHelper().isEdgeTouched(ViewDragHelper.EDGE_RIGHT, pointerId);
                case RIGHT:
                    return draggerView.getDragHelper().isEdgeTouched(ViewDragHelper.EDGE_LEFT, pointerId);
                case TOP:
                    return draggerView.getDragHelper().isEdgeTouched(ViewDragHelper.EDGE_BOTTOM, pointerId);
                case BOTTOM:
                    return draggerView.getDragHelper().isEdgeTouched(ViewDragHelper.EDGE_TOP, pointerId);
                default:
                    return draggerView.getDragHelper().isEdgeTouched(ViewDragHelper.EDGE_LEFT, pointerId);
            }
        } else {
        ...
        switch (draggerView.getDragPosition()) {
            case RIGHT:
                if (left > 0) {
                    ...
                    rightBound = draggerListener.getDraggerViewWidth();
                }
                break;
            case LEFT:
                if (left < 0) {
                    leftBound = -draggerListener.getDraggerViewWidth();
        ...
        switch (draggerView.getDragPosition()) {
            case TOP:
                if (top > 0) {
                    ...
                    bottomBound = draggerListener.getDraggerViewHeight();
                }
                break;
            case BOTTOM:
                if (top < 0) {
                    topBound = -draggerListener.getDraggerViewHeight();
    ...
    public int getViewHorizontalDragRange(View child) {
        return draggerListener.getDraggerViewWidth();
    }
    ...
    public int getViewVerticalDragRange(View child) {
        return draggerListener.getDraggerViewHeight();
    }
    ...
    public void onViewDragStateChanged(int state) {
        ...
        if ((dragState == ViewDragHelper.STATE_DRAGGING
                ...
                && (dragOffset == draggerListener.getDraggerViewWidth()
                || dragOffset == draggerListener.getDraggerViewHeight())) {
    ...
    public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
        ...
        switch (draggerView.getDragPosition()) {
            ...
            case BOTTOM:
                ...
                fractionScreen = (float) dragOffset / draggerListener.getDraggerViewHeight();
                break;
            case LEFT:
            case RIGHT:
            default:
                dragOffset = Math.abs(left);
                fractionScreen = (float) dragOffset / draggerListener.getDraggerViewWidth();
                break;
        }
    ...
    public void onViewReleased(View releasedChild, float xVel, float yVel) {
        ...
        if (draggerView.isDragViewAboveTheLimit()) {
            switch (draggerView.getDragPosition()) {
                case LEFT:
                    draggerView.closeFromCenterToLeft(draggerListener.getDraggerViewWidth());
                    break;
                case RIGHT:
                    draggerView.closeFromCenterToRight(draggerListener.getDraggerViewWidth());
                    break;
                case TOP:
                default:
                    draggerView.closeFromCenterToBottom(draggerListener.getDraggerViewHeight());
                    break;
                case BOTTOM:
                    draggerView.closeFromCenterToTop(draggerListener.getDraggerViewHeight());
                    break;
            }
        } else {
