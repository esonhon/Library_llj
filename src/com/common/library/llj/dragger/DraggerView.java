...
public class DraggerView extends FrameLayout {
    ...
    private boolean useDefaultSpringAnimWhenStartActivity = true;
    ...
    protected void onFinishInflate() {
        ...
        if (!isInEditMode()) {
            ...
            if (isUseDefaultSpringAnimWhenStartActivity())
                preparePosition();
        }
    }
...
//        setHorizontalDragRange(width);
    ...
    public boolean isUseDefaultSpringAnimWhenStartActivity() {
        return useDefaultSpringAnimWhenStartActivity;
    }

    public void setUseDefaultSpringAnimWhenStartActivity(boolean useDefaultSpringAnimWhenStartActivity) {
        this.useDefaultSpringAnimWhenStartActivity = useDefaultSpringAnimWhenStartActivity;
    }
    ...
    public void setEdgeSize(int edgeSize) {
        mEdgeSize = edgeSize;
        dragHelper.setEdgeSize(edgeSize);
    }
    ...
    private void initializeAttributes(AttributeSet attrs) {
        ...
        this.dragPosition = DraggerPosition.getDragPosition(attributes.getInt(R.styleable.dragger_layout_drag_position, DEFAULT_DRAG_POSITION));
        ...
        this.useDefaultSpringAnimWhenStartActivity = attributes.getBoolean(R.styleable.dragger_layout_drag_useDefaultSpringAnimWhenStartActivity, true);
    ...
    private void configDragViewHelper() {
        dragHelper = ViewDragHelper.create(this, SENSITIVITY, new DraggerHelperCallback(this, draggerListener));
    }
    ...
    public void closeActivity() {
        if (dragPosition != null) {
            switch (dragPosition) {
                case LEFT:
                    closeFromCenterToLeft(dragView.getMeasuredWidth());
                    break;
                case RIGHT:
                    closeFromCenterToRight(dragView.getMeasuredWidth());
                    break;
                case TOP:
                default:
                    closeFromCenterToBottom(dragView.getMeasuredHeight());
                    break;
                case BOTTOM:
                    closeFromCenterToTop(dragView.getMeasuredHeight());
                    break;
            }
        } else {
    ...
    public void closeFromCenterToRight(int toX) {
        smoothSlideTo(dragView, toX, 0);
        notifyClosed();
    }

    public void closeFromCenterToLeft(int toX) {
        smoothSlideTo(dragView, -toX, 0);
        notifyClosed();
    }

    public void closeFromCenterToTop(int toY) {
        smoothSlideTo(dragView, 0, -toY);
        notifyClosed();
    }

    public void closeFromCenterToBottom(int toY) {
        smoothSlideTo(dragView, 0, toY);
        notifyClosed();
    }
    ...
    /**
     * ViewDragHelper.STATE_IDLE的时候调用
     */
    private void finish() {
    ...
    private DraggerHelperListener draggerListener = new DraggerHelperListener() {
        ...
        @Override
        public int getDraggerViewWidth() {
            return dragView.getMeasuredWidth();
        }

        @Override
        public int getDraggerViewHeight() {
