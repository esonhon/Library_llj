    ...
    public static ArrayList<VideoInfo> getMediaStoreVideos(Context context, long maxVideoDuring, String videoType) {
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                if (FileUtil.fileIsExist(filePathTemp) && filePathTemp.endsWith(videoType) && during > minVideoDuring && during <= maxVideoDuring) {
                    //文件路径
                    ...
                    if (info.width == 0 || info.height == 0) {
                        ...
                        if (resolution != null) {
                            ...
                            if (split.length == 2) {
                                info.width = ParseUtil.parseInt(split[0]);
                                info.height = ParseUtil.parseInt(split[1]);
                            }
                        }
                    }
                    //经纬度
                    info.longitude = NumberUtil.doubleStringToInteger(cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Video.Media.LONGITUDE)));
                    info.latitude = NumberUtil.doubleStringToInteger(cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Video.Media.LATITUDE)));
                    //文件时长
...
//                        info.thumbPreviewPath = BitmapUtilLj.bitmapToFile(bitmap, new File(thumbnailPath), 100).getAbsolutePath();
//                        bitmap.recycle();
