...
public class InputMethodManagerUtil {
    ...
    public static void hideSoftInputFromWindow(Context context) {
        InputMethodManager inputMethodManager = ((InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE));
        inputMethodManager.hideSoftInputFromWindow(((Activity) context).getWindow().getDecorView().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
    }


    /**
     * 隐藏输入法,默认使用该方法,不用上面那种
     *
     * @param view
     */
