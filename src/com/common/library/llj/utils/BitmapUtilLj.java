...
public class BitmapUtilLj {
    ...
    public static Bitmap.CompressFormat getCompressFormat(String filePath) {
        return StringUtil.isJpgPath(filePath) ? Bitmap.CompressFormat.JPEG : Bitmap.CompressFormat.PNG;
    }
...
//    }
    ...
    public static int[] getBitmapFileSize(String bitmapFilePath) {
        int[] hw = new int[]{0, 0};
        if (FileUtil.fileIsExist(bitmapFilePath)) {
    ...
    public static void createImageThumbnail(Context context, String largeImagePath, String thumbFilePath, int quality, int allPx) throws Exception {
        if (FileUtil.fileIsExist(largeImagePath)) {
            BitmapFactory.Options opts = new BitmapFactory.Options();
            opts.inJustDecodeBounds = true;
            ...
            int degree = com.common.library.llj.utils.BitmapUtilLj.getBitmapDegree(new File(largeImagePath));
            if (degree != 0) {
                bitmap = BitmapUtilLj.rotateBitmap(bitmap, degree);
            }
            //保存到文件中
            BitmapUtilLj.saveBitmapToSD(thumbFilePath, bitmap, Bitmap.CompressFormat.JPEG, quality);
            if (bitmap != null && !bitmap.isRecycled()) {
        ...
        if (FileUtil.fileIsExist(largeImagePath)) {
            ...
            if (opts.outWidth * height > opts.outHeight * width) {
                ...
                if (width > opts.outWidth) {
                    //向着屏幕宽度的比例放大
                    bitmap = BitmapFactory.decodeFile(largeImagePath, opts);
                    bitmap = BitmapUtilLj.scaleBitmapWidth(bitmap, width);
                } else {
                ...
                if (height > opts.outHeight) {
                    //向着屏幕高度的比例放大
                    bitmap = BitmapFactory.decodeFile(largeImagePath, opts);
                    bitmap = BitmapUtilLj.scaleBitmapByHeight(bitmap, height);
                } else {
                    //直接从bitmap中decode对应比例的照片
                    //从照片原始宽度压缩到目标view的宽度,高度会自动按对应比例压缩
                    opts.inDensity = opts.outHeight;
